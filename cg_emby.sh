...
source <(curl -sL https://raw.githubusercontent.com/lihen233/script-store/master/script/script_option)
setcolor
...
bak_emby() {
  ...
  rclone move emby_bak_"$(date "+%Y-%m-%d")".tar "$my_remote": -vP #上传od
...
main_menu() {
  ...
  case $Mainmenu in
    Install_standard)
      ...
      ;;
    Install_Unattended)
      whiptail --clear --ok-button "回车开始执行" --backtitle "Hi,欢迎使用cg_toolbox。有关脚本问题，请访问: https://github.com/cgkings/script-store 或者 https://t.me/cgking_s (TG 王大锤)。}
